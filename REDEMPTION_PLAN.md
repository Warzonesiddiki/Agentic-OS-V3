...
- [x] 13.1. Extract `a2a-server` source module from `gemini-cli/packages/a2a-server/src/` into `packages/a2a-server/src/`.
- [x] 13.2. Create clean `packages/a2a-server/package.json` with correct ESM exports and workspace references.
- [x] 13.3. Add `packages/a2a-server` to `pnpm-workspace.yaml` and root `package.json`.
- [x] 13.4. Resolve TypeScript compilation errors in the extracted A2A server package.
- [x] 13.5. Expose standardized `/.well-known/agent.json` discovery endpoint per the Google A2A specification.
- [x] 13.6. Implement task submission endpoint (`POST /api/v1/a2a/tasks`) accepting remote agent payloads.
- [x] 13.7. Implement task status endpoint (`GET /api/v1/a2a/tasks/:id`) returning real-time progress.
- [x] 13.8. Implement local agent discovery endpoint (`GET /api/v1/a2a/agents`) listing capabilities.
- [x] 13.9. Implement Server-Sent Events (SSE) streaming handler for real-time task log output to calling agents.
- [x] 13.10. Wire incoming A2A tasks to spawn local Agent Runtime process instances.
- [x] 13.11. Implement request signature verification and bearer token auth for A2A endpoints.
- [x] 13.12. Implement outbound A2A client allowing local agents to discover and delegate subtasks to external A2A agents.
- [x] 13.13. Write A2A protocol integration test suite (`server/tests/a2a.test.ts`).
- [x] 13.14. Implement error mapping and timeout handling for inter-agent network calls.
- [x] 13.15. Log all inter-agent messages and task transfers to the audit log.
- [x] 13.16. Completely remove vendored `gemini-cli/` source folder once `packages/a2a-server` is fully integrated.
- [x] 13.17. Update `pnpm-workspace.yaml` to ensure clean workspace linking.
- [x] 13.18. Document A2A inter-agent protocol integration in `docs/HERMES.md` / `docs/A2A_PROTOCOL.md`.
- [x] 13.19. Confirm `pnpm run typecheck` and `pnpm run build` succeed across all workspace packages.
- [x] 13.20. Git checkpoint: `feat: phase 13 — google a2a inter-agent protocol server`.
...
- [x] 16.1. Audit root `Dockerfile` and `docker-compose.yml`.
- [x] 16.2. Build multi-stage production `Dockerfile` for backend server (build stage -> lightweight Node 20 alpine runtime stage).
- [x] 16.3. Build multi-stage production `Dockerfile.frontend` building Vite SPA and serving via Nginx alpine.
- [x] 16.4. Configure root `docker-compose.yml` orchestrating PostgreSQL, Redis, Server, and Frontend containers.
- [x] 16.5. Configure Nginx reverse proxy routing `/api/*` to backend server and static assets to frontend.
- [x] 16.6. Update `.env.example` with full documentation for every production environment variable.
- [x] 16.7. Configure persistent named Docker volumes for Postgres data, Redis data, and SQLite backups.
- [x] 16.8. Add container healthchecks (`curl -f http://localhost:9900/api/v1/health`) for all services in docker-compose.
- [x] 16.9. Enforce CPU and memory resource constraints on container services in docker-compose.
- [x] 16.10. Configure non-root user execution (`USER node`) in backend Dockerfile for container hardening.
- [x] 16.11. Implement graceful SIGTERM handling in server startup (`index.ts`) draining connections before exit.
- [x] 16.12. Build `docker-compose.dev.yml` supporting local hot-reloading and source volume mounting.
- [x] 16.13. Build standalone single-container Dockerfile operating in lightweight SQLite-only mode.
- [x] 16.14. Implement environment validation on server startup logging clear error messages if required vars are missing.
- [x] 16.15. Add auto-migration execution to container startup entrypoint script.
- [x] 16.16. Test clean container stack deployment (`docker compose up --build -d`).
- [x] 16.17. Verify `/api/v1/health` returns HTTP 200 OK from running container stack.
- [x] 16.18. Document container deployment workflows in `docs/DEPLOYMENT.md`.
- [x] 16.19. Verify full clean checkout builds and launches via Docker without errors.
- [x] 16.20. Git checkpoint: `ops: phase 16 — docker containerization and compose orchestration`.
