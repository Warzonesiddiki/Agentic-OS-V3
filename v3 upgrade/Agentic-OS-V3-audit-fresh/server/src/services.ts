...
import { memories, skills, projects, feedback, systemMeta } from "./db/schema.js";
...
async function assertOperational(): Promise<void> {
  if (await isKillSwitchOn()) throw new ApiError("SAFETY_KILL_SWITCH", "Kill switch is engaged — mutations are blocked.");
}
...
export async function createMemory(input: MemoryRow, actor: string): Promise<unknown> {
  ...
  return db.transaction(async (tx: Tx) => {
    ...
    try {
      ...
      log.warn("auto_embed_failed_create", { title: input.title, error: e instanceof Error ? e.message : String(e) });
    }

    const [created] = await tx.insert(memories).values({
      id: `mem_${randomUUID()}`,
      kind: input.kind, title: input.title, content: input.content, tags: input.tags,
      importance: input.importance, source: input.source, projectId: input.projectId,
      tokenCost: estimateTokens(input.content), recallCount: 0,
      embedding,
    }).returning();
    if (!created) throw new Error("Failed to create memory — DB returned no row.");
    await appendAudit("memory.created", { id: created.id, kind: created.kind, title: created.title, embedded: Boolean(embedding) }, actor, tx);
    return created;
  });
}

export async function updateMemory(id: string, patch: Partial<MemoryRow>, actor: string): Promise<unknown> {
  await assertOperational();
  return db.transaction(async (tx: Tx) => {
    ...
    if (patch.content) { updates.content = patch.content; updates.tokenCost = estimateTokens(patch.content); }
    ...
    if ((patch.title || patch.content) && !updates.embedding) {
      try {
        const { embedQuery } = await import("./services/embeddings.js");
        const newText = `${patch.title ?? existing.title} ${patch.content ?? existing.content}`.slice(0, 8000);
        const emb = await embedQuery(newText);
        if (emb) updates.embedding = emb;
      } catch (e) {
        const { log } = await import("./lib/logging.js");
        log.warn("auto_embed_failed_update", { id, error: e instanceof Error ? e.message : String(e) });
      }
    }
...
export async function deleteMemory(id: string, actor: string): Promise<void> {
  ...
  await db.transaction(async (tx: Tx) => {
    const [deleted] = await tx.delete(memories).where(eq(memories.id, id)).returning({ id: memories.id });
...
export async function createSkill(input: SkillRow, actor: string): Promise<unknown> {
  ...
  return db.transaction(async (tx: Tx) => {
    const [created] = await tx.insert(skills).values({
      id: `skl_${randomUUID()}`,
      name: input.name, title: input.title, description: input.description, content: input.content,
      category: input.category, tags: input.tags, trigger: input.trigger, source: input.source, projectId: input.projectId,
    }).returning();
...
export async function updateSkill(id: string, patch: Partial<SkillRow>, actor: string): Promise<unknown> {
  await assertOperational();
  return db.transaction(async (tx: Tx) => {
 ...
 */
export async function recordOutcome(id: string, outcome: "success" | "failure", actor: string): Promise<unknown> {
  await assertOperational();
  return db.transaction(async (tx: Tx) => {
...
export async function transferProject(input: {
  ...
  memories?: { kind: string; title: string; content: string; tags?: string[]; importance?: number }[];
  skills?: SkillRow[];
  ...
  const { memCreated, sklUpserted } = await db.transaction(async (tx: Tx) => {
    ...
    for (const m of input.memories ?? []) {
      const [row] = await tx.insert(memories).values({
        id: `mem_${randomUUID()}`, kind: m.kind, title: m.title, content: m.content,
        tags: m.tags ?? [], importance: m.importance ?? 0.6, source: "transfer", projectId: project.id,
        tokenCost: estimateTokens(m.content), recallCount: 0,
      }).returning();
      if (row) mc++;
    }
    for (const s of input.skills ?? []) {
      const existing = await tx.query.skills.findFirst({ where: and(eq(skills.name, s.name), eq(skills.projectId, project.id)) });
      if (existing) {
        await tx.update(skills).set({ title: s.title, description: s.description, content: s.content, category: s.category, updatedAt: new Date() }).where(eq(skills.id, existing.id));
      } else {
        await tx.insert(skills).values({ id: `skl_${randomUUID()}`, name: s.name, title: s.title, description: s.description, content: s.content, category: s.category, tags: s.tags, source: "transfer", projectId: project.id });
      }
      sc++;
    }
    await appendAudit("project.transferred", { projectId: project.id, projectName: input.projectName, memCreated: mc, sklUpserted: sc }, actor, tx);
    return { memCreated: mc, sklUpserted: sc };
  });
  return { projectId: project.id, created: project.created, memCreated, sklUpserted };
}

export async function checkpoint(label: string, context: string, projectName: string | undefined, actor: string): Promise<unknown> {
  await assertOperational();
  const projectId = projectName ? (await ensureProject(projectName, "checkpoint")).id : null;
  return db.transaction(async (tx: Tx) => {
    const [row] = await tx.insert(memories).values({
      id: `mem_${randomUUID()}`, kind: "episodic", title: label.slice(0, 200), content: context,
      tags: ["checkpoint"], importance: 0.6, source: "checkpoint", projectId,
      tokenCost: estimateTokens(context), recallCount: 0,
    }).returning();
 ...
 */
export async function captureSession(transcript: string, projectName: string | undefined, actor: string, forceFail = false): Promise<CaptureReport> {
  await assertOperational();
  ...
  try {
    ...
    const created = await db.transaction(async (tx: Tx) => {
      ...
      for (const d of distilled) {
        const [row] = await tx.insert(memories).values({
          id: `mem_${randomUUID()}`, kind: d.kind, title: d.title.slice(0, 200), content: d.content,
          tags: d.tags, importance: d.importance, source: "session", projectId,
          tokenCost: estimateTokens(d.content), recallCount: 0,
        }).returning();
        rows.push(row);
      }
      await appendAudit("session.captured", { distilled: true, memories: rows.length, projectId }, actor, tx);
      return rows;
    });
    ...
    const rawProjectId = projectId ?? (projectName ? (await ensureProject(projectName, "session-raw").catch(() => ({ id: null as string | null }))).id : null);
    await db.transaction(async (tx: Tx) => {
      const [raw] = await tx.insert(memories).values({
        id: `mem_${randomUUID()}`, kind: "episodic", title: "Session transcript (undistilled)",
        content: transcript, tags: ["session", "undistilled", "preserved"], importance: 0.5,
        source: "session-raw", projectId: rawProjectId, tokenCost: estimateTokens(transcript), recallCount: 0,
      }).returning();
      if (!raw) throw new Error("Failed to store undistilled transcript — DB returned no row.");
      await appendAudit("session.captured", { distilled: false, transcriptPreserved: true, reason, rawMemoryId: raw.id }, actor, tx);
    });
    return { distilled: false, transcriptPreserved: true, memories: 1, transcript, reason };
  }
}

export async function ensureProject(name: string, source: string): Promise<{ id: string; created: boolean }> {
  ...
  if (!existing) throw new Error(`Project "${name}" exists but could not be read after conflict resolution.`);
  return { id: existing.id, created: false };
}

export async function setKillSwitch(enabled: boolean, reason: string | undefined, actor: string): Promise<void> {
  await db.transaction(async (tx: Tx) => {
    const value = enabled ? "1" : "0";
    await tx.insert(systemMeta).values({ key: "killSwitch", value, updatedAt: new Date() }).onConflictDoUpdate({ target: systemMeta.key, set: { value, updatedAt: new Date() } });
    if (reason != null) {
      await tx.insert(systemMeta).values({ key: "killSwitchReason", value: reason, updatedAt: new Date() }).onConflictDoUpdate({ target: systemMeta.key, set: { value: reason, updatedAt: new Date() } });
    }
    await appendAudit(enabled ? "safety.kill_switch.engaged" : "safety.kill_switch.released", { reason: reason ?? null }, actor, tx);
  });
}

export async function recordFeedback(input: { query: string; itemId: string; itemType: string; helpful: boolean }, actor: string): Promise<void> {
  await db.transaction(async (tx: Tx) => {
    await tx.insert(feedback).values({ id: `fb_${randomUUID()}`, query: input.query, itemId: input.itemId, itemType: input.itemType, helpful: input.helpful });
    await appendAudit("feedback.recorded", { itemId: input.itemId, helpful: input.helpful }, actor, tx);
  });
}
