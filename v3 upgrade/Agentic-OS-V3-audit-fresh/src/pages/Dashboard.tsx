...
export default function Dashboard({ setPage }: { setPage: (p: PageId) => void }) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold tracking-tight text-slate-100">NEXUS 2.0 — Operating System for AI Agents</h1>
        <p className="mt-1 text-sm text-slate-400">
          Persistent memory, token-budgeted recall, skills, governance and an MCP surface — a second brain your agents actually use.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
        ...
        <Stat label="Tokens saved" value={formatCompact(tokensSaved)} sub="reused knowledge" tone="emerald" />
        <Stat label="Audit entries" value={formatNumber(s.audit.length)} sub={audit.valid ? "chain valid" : "BROKEN"} tone={audit.valid ? "emerald" : "rose"} />
      </div>

      <div className="grid gap-6 lg:grid-cols-3">
        <Card className="p-5 lg:col-span-2">
          <SectionTitle title="System health" subtitle="Live snapshot of the brain and its subsystems" />
          <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3">
            <Health label="Store" value="reachable" ok />
            <Health label="Audit chain" value={audit.valid ? `valid · ${audit.verifiedEntries}` : `broken @ ${audit.brokenAt}`} ok={audit.valid} />
            <Health label="LLM mode" value={llmMode()} ok={llmMode() === "configured"} warn={llmMode() !== "configured"} />
            <Health label="Recall engine" value="BM25 lexical" ok />
            <Health label="Kill switch" value={s.meta.killSwitch === "1" ? "ENGAGED" : "disengaged"} ok={s.meta.killSwitch !== "1"} />
            <Health label="Heartbeat" value={drift < 60000 ? "fresh" : "stale"} ok={drift < 60000} warn={drift >= 60000} />
          </div>
          <div className="mt-5">
            <div className="mb-2 text-xs font-medium text-slate-400">Ambient context <span className="text-slate-600">({amb.tokens} tokens)</span></div>
            <CodeBlock>{amb.text}</CodeBlock>
          </div>
        </Card>

        <Card className="p-5">
          <SectionTitle title="Recent activity" subtitle="Hash-chained audit tail" action={<button onClick={() => setPage("audit")} className="text-xs text-cyan-400 hover:underline">view all →</button>} />
          <div className="mt-3 space-y-2">
            {recent.map((e) => (
              <div key={e.id} className="flex items-center gap-3 rounded-lg border border-nexus-border bg-slate-950/40 px-3 py-2">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-[10px] text-slate-600">{e.actor} · {timeAgo(e.createdAt)}</div>
                </div>
                <span className="font-mono text-[9px] text-slate-700">{shortHash(e.entryHash, 7)}</span>
              </div>
            ))}
      ...
      <Card className="p-5">
        <SectionTitle title="Connect an agent (MCP)" subtitle="Point any MCP client at the local endpoint" action={<button onClick={() => setPage("docs")} className="text-xs text-cyan-400 hover:underline">full docs →</button>} />
        <p className="mt-2 text-xs text-slate-500">Every tool validates input with Zod, maps to a scope, respects the kill switch, and never bypasses REST security.</p>
        <div className="mt-3">
          <CodeBlock>{`# Endpoint
...
function Health({ label, value, ok, warn }: { label: string; value: string; ok?: boolean; warn?: boolean }) {
...
function ServerStatus() {
  const [status, setStatus] = useState<{ ok: boolean; status?: string; error?: string } | null>(null);
  ...
  useEffect(() => {
    if (!remote.enabled) { setStatus(null); return; }
    remoteApi.ping().then(setStatus).catch(() => setStatus({ ok: false, error: "ping failed" }));
  ...
  return (
    <Card className="p-4">
      ...
      <p className="mt-2 text-[11px] text-slate-500">
        When connected, the dashboard can read/write through the server's REST API.
        Currently: local engine only. Remote data path is planned for full integration.
      </p>
    </Card>
  );
}
