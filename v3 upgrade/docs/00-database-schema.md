# 00 — Complete Database Schema
## NEXUS V3 — All 16 Tables

> **This file contains the COMPLETE Drizzle ORM schema.**
> Copy this entire file to `server/src/db/schema.ts`.

```typescript
// server/src/db/schema.ts
// NEXUS V3 — Complete schema with pgvector, HNSW, FKs, unique indexes
...
export const vector = (dimension: number) =>
  customType<{ data: number[]; driverData: string; config: { dimension: number } }>({
    ...
    fromDriver(value: string): number[] {
      return value.replace(/[[\]"]/g, "").split(",").filter((s) => s.trim() !== "").map(Number);
    },
  })(`embedding`, { dimension });
...
export const notes = pgTable("notes", {
  ...
  // V3 FIX: Add embedding column to notes (was missing in V2)
  embedding: vector(1536),
}, (t) => ({
...
## V3 Schema Fixes Applied

1. **`notes.embedding` added** — V2 forgot this; notes can now participate in semantic recall
2. **`notes` HNSW index added** — for fast ANN search on notes
3. **All FK relationships defined** — `memories.projectId` and `skills.projectId` reference `projects.id`
4. **COALESCE unique index on skills** — NULL-safe uniqueness for skill names within projects
5. **`compiledScripts` table** — for neural skill compilation
6. **All 16 tables present** — matches the full architecture
...
docker compose exec postgres psql -U postgres -d nexus -c "\dt"
# Expected: 16 tables listed

# Verify pgvector works
docker compose exec postgres psql -U postgres -d nexus -c "
  SELECT typename FROM pg_type WHERE typename = 'vector';
"
# Expected: one row with typename = 'vector'
```
