...
A **DevOps engineer or AI operations lead** who manages a team of 50+ AI agents across multiple projects. They need situational awareness, fast diagnostics, and surgical control. They are comfortable in terminals but need a visual layer for comprehensibility. They trust the system but need to _verify_ it.
...
## 9. Native GUI Desktop Actuator Prerequisites & OS Permissions

The Desktop Actuator (`server/src/services/desktop-actuator.ts`) provides visual GUI automation for VLM agents across Windows, macOS, Linux, and Headless fallback environments.

### Platform Requirements & Prerequisites

| Operating System      | Backend Mechanism                           | CLI / Binary Dependencies                    | OS Permissions Required                                                                        |
| --------------------- | ------------------------------------------- | -------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| **Windows**           | PowerShell + `System.Windows.Forms`         | `powershell` (pre-installed)                 | User context with GUI desktop session                                                          |
| **macOS**             | AppleScript (`osascript`) + `screencapture` | `osascript`, `screencapture` (pre-installed) | **Accessibility** & **Screen Recording** permissions in _System Settings → Privacy & Security_ |
| **Linux**             | `xdotool` + ImageMagick `import`            | `xdotool`, `imagemagick` (`import` CLI)      | Active X11/Wayland desktop display session (`$DISPLAY`)                                        |
| **Headless / Docker** | `HeadlessActuator`                          | None                                         | Safe stub mode returning log entries and 1x1 PNG base64 stubs                                  |

### Manual Override & Configuration

- **Mode Override:** Set environment variable `NEXUS_GUI_MODE=windows|mac|linux|headless` to force a specific actuator backend.
- **Input Rate Limiting:** Enforced at maximum 10 GUI input actions / second (100ms minimum interval) to prevent runaway input loops.
- **Command Sanitization:** All text inputs and arguments are sanitized to eliminate shell injection vulnerabilities.

---

_End of UX Spec — Prism. Awaiting Atlas MASTER_SPEC §6 for implementation alignment._
