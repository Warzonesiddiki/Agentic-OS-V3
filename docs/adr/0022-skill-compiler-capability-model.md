...
- **Guardrail seam:** compilation/activation counts feed the self-opt harness
  (ADR-0014) and are bounded by Sentinel guardrails (token-spend ceiling).
...
- Tests: `skill-compiler.test.ts` covers pattern detection, eval-match gate,
  capability deny, and audit append on activation.
