...
- **Realized seam (verified 2026-07-09):** the auto-tuner applies live only via
  the *actually exported* setters — `configureWorker`, `setConcurrency`,
  `setWorkerConcurrency`, `setMaintenance`, `setWorkerTimeout`, `prewarmCache`
  (Forge/task-worker), `setSchedulingPolicy` (Forge/scheduler),
  `hotpatchModule` (Forge/kernel), `setGuardrailThreshold` (Sentinel/guardrails).
  Names like `applySchedulerBoost` / `applyAgentRestartPolicy` are NOT exported;
  tuners must call the verified setters or degrade to advisory noop.
