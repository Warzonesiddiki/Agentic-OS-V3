# E10-S23 — Build Deterministic R1 Integration Harness

**Epic:** E10-R1 — Integrity, Security, and Release Requalification
**Priority:** P0
**Status:** backlog
**Accountable owner:** @quill (test infrastructure)
**Dependencies:** E10-S22
**Release authority:** None. R1 remains blocked until an independent E10-S30 approval.

## User story

As an R1 test author, I need deterministic real runtime composition, so integration evidence does not depend on private fields, ambient clocks, or unbounded processes.

## Scope and acceptance criteria

1. Compose real routes/services/repositories with controlled filesystem, database, clock, process, identity, and cleanup fixtures.
2. Support SQLite/PGlite/PostgreSQL selection, project-root fixtures, controlled runner effects, and fault injection without private-field mutation.
3. Make tests parallel-safe and leak-check processes/files/databases.
4. Expose only typed test seams that cannot become production bypasses.
5. Document fixture setup/teardown and environment prerequisites.

## Negative and adversarial cases

- Do not test critical recovery through an in-memory fake when a supported real adapter exists.
- No test fixture may inherit developer secrets or write outside controlled roots.

## Delivery tasks

- [ ] Design fixture lifecycle.
- [ ] Implement adapters and cleanup checks.
- [ ] Migrate R1 integration tests.
- [ ] Measure determinism/flakiness and review seams.
...
- `server/tests/helpers/r1-integration-harness.ts`
- `server/tests/r1-integration-harness.test.ts`
