# E10-S28 — Complete Security Triage and Penetration Matrix

**Epic:** E10-R1 — Integrity, Security, and Release Requalification
**Priority:** P0
**Status:** backlog
**Accountable owner:** @sentinel (security review)
**Dependencies:** E10-S27
**Release authority:** None. R1 remains blocked until an independent E10-S30 approval.

## User story

As a security decision maker, I need a formal finding matrix and retest evidence, so remediation priorities, exceptions, and residual risk are explicit.

## Scope and acceptance criteria

1. Record every audit, dependency, sandbox, tenant, auth, export, and penetration finding with ID, severity, exploitability, affected version/path, owner, due date, mitigation, status, and retest evidence.
2. Define severity/SLA/exception approval rules and require expiry/retest for accepted risk.
3. Include adversarial tests for all R1 trust boundaries and map them to results.
4. Redact secrets and sensitive exploit details from public evidence while retaining actionable private references.
5. The matrix must expose unresolved blockers to the release ledger.

## Negative and adversarial cases

- “Low risk” without exploitability, owner, date, and retest is not triage.
- A passed unit test cannot close a dependency advisory without applicability analysis.

## Delivery tasks

- [ ] Create finding inventory and scoring rules.
- [ ] Populate owner/due date/mitigation.
- [ ] Run retests.
...
- `docs/bmad/releases/R1-security-triage-matrix.md`
