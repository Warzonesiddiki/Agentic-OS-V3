# E10-S10 — Implement Governed Stale-Claim Reconciliation Workflow

**Epic:** E10-R1 — Integrity, Security, and Release Requalification
**Priority:** P0
**Status:** in_progress
**Accountable owner:** @forge (worker recovery)
**Dependencies:** E10-S9
**Release authority:** None. R1 remains blocked until an independent E10-S30 approval.

## User story

As an administrator, I need to review and reconcile uncertain stale effects, so crashes after an external effect are governed rather than automatically replayed.

## Scope and acceptance criteria

1. Model claimed, effect-observed, receipt-recorded, completed, failed, and reconciliation-required states with allowed/terminal transitions.
2. List stale claims only to an authorized principal in the same project and correlate task, checkpoint, approval, claim, receipt, actor, and evidence.
3. Provide explicit admin-reviewed reconciliation actions with reason, evidence reference, auditable decision, and concurrency-safe transition.
4. Allow requeue only after evidence proves no external effect occurred; never automatically replay an uncertain non-transactional effect.
5. Return typed, Zod-validated request/response contracts and safe errors.

## Negative and adversarial cases

- GET recovery visibility alone is not reconciliation.
- A stale claim cannot be silently completed, discarded, or re-executed.

## Delivery tasks

- [ ] Define state machine and repository methods.
- [ ] Implement authorize/review/reconcile route and receipts.
- [ ] Add cross-project, stale-race, and invalid-transition tests.
- [ ] Document admin runbook and review evidence.
