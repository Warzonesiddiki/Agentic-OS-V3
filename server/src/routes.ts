...
import { memoryInput, memoryPatch, skillInput, recallQuery, captureInput, outcomeInput, killSwitchInput, feedbackInput } from "./lib/schemas.js";
import { createMemory, updateMemory, deleteMemory, createSkill, updateSkill, deleteSkill, recordOutcome, captureSession, setKillSwitch, recordFeedback, transferProject, checkpoint, isKillSwitchOn } from "./services.js";
...
api.get("/api/v1/system", async (c) => {
  ...
  return c.json(ok({
    ...
    counts: { memories: mem[0]?.n ?? 0, skills: skl[0]?.n ?? 0, projects: prj[0]?.n ?? 0, audit: aud[0]?.n ?? 0 },
  }, c.get("requestId") ?? ""));
});
...
api.get("/api/v1/memories", async (c) => {
  ...
  const items = cursor
    ? await db.query.memories.findMany({ limit: limit + 1, where: gt(memories.id, cursor), orderBy: memories.id })
    : await db.query.memories.findMany({ limit: limit + 1, orderBy: memories.id });
  const hasMore = items.length > limit;
  if (hasMore) items.pop();
  return c.json(ok({ total: items.length, items, limit, cursor, hasMore, nextCursor: hasMore ? items[items.length - 1]?.id : undefined }, c.get("requestId") ?? ""));
});
...
api.get("/api/v1/recall", async (c) => {
  ...
  const result = await recall(q.q, q.budget as number, p.name, { cursor: q.cursor, limit: q.limit });
...
api.post("/api/v1/recall/conversation", async (c) => {
  const p = await requireScope(c, "memory:read");
  const body = parse(z.object({ query: z.string().min(1), budget: z.number().int().min(64).max(8192).default(1500) }), await safeJson(c));
...
api.post("/api/v1/checkpoint", async (c) => {
  const p = await requireScope(c, "memory:write");
  const body = parse(z.object({ label: z.string().min(1).max(160), context: z.string().min(1), projectName: z.string().optional() }), await safeJson(c));
...
api.post("/api/v1/projects/transfer", async (c) => {
  ...
  const body = parse(z.object({
    ...
    memories: z.array(z.object({
      ...
      title: z.string().min(1), content: z.string().min(1), tags: z.array(z.string()).default([]), importance: z.number().default(0.6),
    })).default([]),
    skills: z.array(z.object({
      name: z.string().min(1), title: z.string().min(1), description: z.string().min(1), content: z.string().min(1),
      category: z.string().default("general"), tags: z.array(z.string()).default([]), trigger: z.string().nullable().default(null), source: z.string().default("transfer"),
    })).default([]),
  }), await safeJson(c));
...
api.post("/api/v1/vault/write-back", async (c) => {
  const p = await requireScope(c, "vault:write");
  const body = parse(z.object({ memoryId: z.string().min(1), path: z.string().max(500).optional() }), await safeJson(c));
...
api.get("/api/v1/ledger", async (c) => {
  ...
  const [items, sumRow] = await Promise.all([
    ...
    db.select({ total: sql<number>`coalesce(sum(tokens_saved), 0)::int`, count: sql<number>`count(*)::int` }).from(tokenLedger),
  ]);
  return c.json(ok({ items, totalSaved: sumRow[0]?.total ?? 0, total: sumRow[0]?.count ?? 0, limit, offset }, c.get("requestId") ?? ""));
});
...
api.get("/api/v1/safety", async (c) => {
  ...
  return c.json(ok({ killSwitch: engaged, killSwitchReason: m.get("killSwitchReason") ?? "", lastHeartbeat: last, heartbeatOk: Date.now() - last < 60_000, llmMode: llmConfigured() ? "configured" : "lexical" }, c.get("requestId") ?? ""));
});
...
api.post("/api/v1/safety/heartbeat", async (c) => {
  const p = await requireScope(c, "safety:write");
  const now = Date.now();
  await db.insert(systemMeta).values({ key: "lastHeartbeat", value: String(now), updatedAt: new Date() }).onConflictDoUpdate({ target: systemMeta.key, set: { value: String(now), updatedAt: new Date() } });
...
api.post("/api/v1/admin/keys", async (c) => {
  ...
  const body = parse(z.object({
    ...
    ] as [Scope, ...Scope[]])).min(1),
  }), await safeJson(c));
  const created = await createPrincipal(db, body.name, body.scopes);
  // Raw key shown exactly once; caller must store it.
  return c.json(ok({ id: created.id, name: body.name, scopes: body.scopes, key: created.rawKey, note: "Store this key now — it will not be shown again." }, c.get("requestId") ?? ""), 201);
});

api.delete("/api/v1/admin/keys/:id", async (c) => {
  await requireScope(c, "brain:admin");
  const revoked = await revokePrincipal(db, c.req.param("id"));
  if (!revoked) return c.json(err("NOT_FOUND", "Principal not found.", c.get("requestId") ?? ""), 404);
...
api.post("/api/v1/audit/trajectory", async (c) => {
  ...
  const body = parse(z.object({
    ...
    tokenUsage: z.object({ prompt: z.number(), completion: z.number(), total: z.number() }).optional(),
...
api.post("/api/v1/compiled-scripts/compile", async (c) => {
  ...
  if (result.activated > 0) {
    broadcastSSE({ type: "audit.appended", data: { event: "skill.compiled", activated: result.activated }, timestamp: Date.now() });
  }
  return c.json(ok(result, c.get("requestId") ?? ""));
});
