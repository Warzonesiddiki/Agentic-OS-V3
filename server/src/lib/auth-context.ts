...
export async function resolvePrincipal(c: Context<NexusEnv>): Promise<Principal | null> {
  ...
  const key = c.req.header("authorization")?.replace(/^Bearer\s+/i, "").trim() ?? null;
...
export async function requireScope(c: Context, scope: Scope) {
  ...
  if (!principal.scopes.includes(scope)) throw new ApiError("FORBIDDEN", `Missing required scope: ${scope}`);
  return principal;
}
...
export function fail(c: Context, e: unknown) {
  ...
  if (e instanceof ApiError) {
    return c.json(err(e.code, e.message, traceId, e.status), e.status as Parameters<typeof c.json>[1]);
  }
