...
export function workerStatus() {
  return { running, activeCount, pollIntervalMs: options.pollIntervalMs, maxConcurrency: options.maxConcurrency, maintenanceIntervalMs: options.maintenanceIntervalMs };
}
...
export function startWorker(actor: string): void {
  if (running) return;
  running = true;
  log.info("worker_started", { pollIntervalMs: options.pollIntervalMs, maxConcurrency: options.maxConcurrency });
...
export function wakeWorker(): void {
  if (!running) return;
  tick("system").catch((e) => { log.error("worker_wake_tick_failed", { error: e instanceof Error ? e.message : String(e) }); });
}
...
async function runMaintenance(actor: string): Promise<void> {
  ...
  const deadAgents = await db.update(agents).set({
    status: "errored",
    updatedAt: now,
  }).where(and(
    eq(agents.status, "thinking"),
    lt(agents.lastHeartbeatAt, heartbeatCutoff),
  ...
  if (_shadowCycleCount % 10 === 0) {
    import("../services/shadow-daemon.js").then((m) => m.runShadowCycle()).catch((e) => { log.error("shadow_cycle_failed", { error: e instanceof Error ? e.message : String(e) }); });
  }
}
...
async function tick(actor: string): Promise<void> {
  ...
  try {
    ...
    log.error("worker_task_panic", { taskId: task.id, error: e instanceof Error ? e.message : String(e) });
...
async function executeTask(task: TaskRow, actor: string): Promise<void> {
  ...
  broadcastSSE({ type: "agent.state", data: { agentId: task.agentId, status: "thinking", taskLabel: task.label }, timestamp: Date.now() });
  broadcastSSE({ type: "task.update", data: { taskId: task.id, status: "running", agentId: task.agentId, label: task.label }, timestamp: Date.now() });
  ...
  try {
    ...
    broadcastSSE({ type: "agent.state", data: { agentId: task.agentId, status: "idle" }, timestamp: Date.now() });
    broadcastSSE({ type: "task.update", data: { taskId: task.id, status: "succeeded", agentId: task.agentId, label: task.label }, timestamp: Date.now() });
    log.info("worker_task_completed", { taskId: task.id, label: task.label, durationMs: Date.now() - start });
  } catch (e) {
    const errorMsg = e instanceof Error ? e.message : String(e);
    await failTask(task.id, errorMsg, actor);
    await updateAgentState(task.agentId, "errored");
    broadcastSSE({ type: "agent.state", data: { agentId: task.agentId, status: "errored", error: errorMsg }, timestamp: Date.now() });
    broadcastSSE({ type: "task.update", data: { taskId: task.id, status: "failed", error: errorMsg, agentId: task.agentId, label: task.label }, timestamp: Date.now() });
    log.warn("worker_task_failed", { taskId: task.id, label: task.label, error: errorMsg, durationMs: Date.now() - start });
  }
}
...
async function dispatchTask(task: TaskRow, actor: string, signal: AbortSignal): Promise<unknown> {
  ...
  if (kind === "interactive" || kind === "background") {
    ...
    if (labelLower.includes("browser") || labelLower.includes("navigate") || labelLower.includes("scrape")) {
    ...
    if (labelLower.includes("research") || labelLower.includes("explore") || labelLower.includes("investigate") || labelLower.includes("analyze")) {
  ...
  if (kind === "maintenance") {
    ...
    if (labelLower.includes("compile") || labelLower.includes("pattern") || labelLower.includes("self-improve")) {
...
async function handleLLM(task: TaskRow, actor: string, signal: AbortSignal): Promise<unknown> {
  ...
  if (agent && agent.tokensUsed >= agent.tokenBudget) {
    throw new Error(`Token budget exhausted for agent ${task.agentId}: ${agent.tokensUsed}/${agent.tokenBudget}`);
  }

  const ctx = await recall(query, contextBudget, actor);
  const contextText = ctx.returned.map((r) => `[${r.type}] ${r.title}\n${r.content.slice(0, 2000)}`).join("\n\n");
...
${contextText}`,
  ...
  await appendAudit("worker.llm_completed", {
    taskId: task.id, model: result.model,
...
async function handleSessionCapture(input: Record<string, unknown>, actor: string): Promise<unknown> {
...
async function handleAmbientDistillation(input: Record<string, unknown>, actor: string): Promise<unknown> {
  ...
  for (const d of distilled.slice(0, 25)) {
    const [row] = await db.insert(memSchema).values({
      id: `mem_${randomUUID()}`,
      kind: d.kind,
      title: d.title,
      content: d.content,
  ...
  await appendAudit("ambient.distilled", { source, memories: rows.length, transcriptLen: transcript.length }, actor);
