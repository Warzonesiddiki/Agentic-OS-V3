...
export async function logTrajectory(
  ...
  actor: string
): Promise<{ auditSequence: number; trajectoryId: string }> {
 ...
 */
export async function logToolReceipt(
  input: ToolReceiptInput,
  actor: string
): Promise<{ receiptId: string; auditSequence: number }> {
  const entry = await appendAudit(
    'agent.tool_executed',
    {
      agentId: input.agentId,
...
export async function verifyAndAutoKill(): Promise<{ healthy: boolean; reason?: string }> {
  ...
  await appendAudit(
    'safety.auto_kill_engaged',
    { reason, brokenAt: result.brokenAt },
    'system-auto'
  );
