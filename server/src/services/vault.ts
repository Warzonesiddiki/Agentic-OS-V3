...
export function parseMarkdown(raw: string): Parsed {
  ...
  if (frontmatter.tags) frontmatter.tags.replace(/[[\]]/g, "").split(",").forEach((t) => t.trim() && tagSet.add(t.trim()));
  for (const m of body.matchAll(/(?:^|\s)#([a-z][a-z0-9_/-]*)/gi)) tagSet.add(m[1]!.toLowerCase());
  const wikilinks = [...new Set([...body.matchAll(/\[\[([^\]|]+).*?\]\]/g)].map((m) => m[1]!.trim()))];
  const title = frontmatter.title || heading?.[1]?.trim() || "untitled";
  return { title, content: body.trim(), frontmatter, tags: [...tagSet], wikilinks };
}

async function walk(dir: string, root: string, acc: { path: string; content: string; mtime: Date }[]): Promise<void> {
  let entries: string[];
  try {
  ...
  for (const name of entries) {
    ...
    if (s.isDirectory()) {
      ...
      acc.push({ path: "/" + relative(root, full).split(/[\\/]/).join("/"), content, mtime: s.mtime });
    }
  }
}
  ...
  for (const f of files) {
    ...
    if (existing) {
      await db.update(notes).set({
        title: parsed.title, content: parsed.content, frontmatter: parsed.frontmatter,
        tags: parsed.tags, wikilinks: parsed.wikilinks, charCount: f.content.length, mtime: f.mtime, indexedAt: new Date(),
      }).where(eq(notes.id, existing.id));
    } else {
      await db.insert(notes).values({
        id: `nte_${randomUUID()}`, path: f.path, title: parsed.title, content: parsed.content,
        frontmatter: parsed.frontmatter, tags: parsed.tags, wikilinks: parsed.wikilinks,
        charCount: f.content.length, mtime: f.mtime, indexedAt: new Date(),
      });
    }
    indexed++;
  }
...
export async function writeBack(memoryId: string, subPath: string | undefined, actor: string): Promise<{ path: string }> {
  const root = env.NEXUS_OBSIDIAN_VAULT;
  if (!root) throw new ApiError("VALIDATION_ERROR", "NEXUS_OBSIDIAN_VAULT is not configured.");
  ...
  if (!safe.ok || !safe.resolved) throw new ApiError("VALIDATION_ERROR", safe.reason ?? "Unsafe write-back path.");
