  ...
  switch (cmd) {
    case "status": {
      ...
      console.log(JSON.stringify({
        ...
        counts: { memories: mem.length, skills: skl.length, projects: prj.length, audit: aud.total },
    ...
    case "remember": {
      ...
      const created = await createMemory({ kind, title, content: body, tags: [], importance: 0.6, source: "cli", projectId: null }, "cli");
      const id = typeof created === "object" && created !== null && "id" in created && typeof (created as { id: unknown }).id === "string" ? (created as { id: string }).id : "unknown";
    ...
    case "capture": {
      ...
      const report = await captureSession(transcript, args.project ? String(args.project) : undefined, "cli");
    ...
    case "doctor": {
      ...
      const checks = [
        { name: "database", level: reachable ? "🟢 ok" : "🔴 broken", detail: reachable ? "reachable" : "unreachable" },
        { name: "audit chain", level: aud.valid ? "🟢 ok" : "🔴 broken", detail: `${aud.verifiedEntries} verified` },
        { name: "llm", level: llmConfigured() ? "🟢 ok" : "🟡 warn", detail: llmConfigured() ? "configured" : "lexical fallback" },
      ];
    ...
    case "keygen": {
      ...
      console.log(JSON.stringify({ key: raw, keyHash: hashApiKey(raw), note: "Store the key; persist the keyHash in api_keys." }, null, 2));
      break;
    }
    case "mcp-config": {
      ...
      const key = envKey || (await createKey(db, "mcp-client", ["memory:read", "memory:write"])).rawKey;
      if (!envKey) console.log(`Created new key for mcp-config: ${key}\n`);
      console.log(JSON.stringify({ mcpServers: { nexus: { url: `${origin}/api/mcp`, headers: { Authorization: `Bearer ${key}` } } } }, null, 2));
      break;
    }
    case "connect": {
...
Usage: nexus connect <agent> [--apply] [--verify]`);
      ...
      if (envKey) {
        ...
        const created = await createPrincipal(db, "hermes", ["memory:read", "memory:write", "skill:read", "skill:write", "audit:read"]);
            ...
            try {
              ...
              try {
                ...
                if (readErr instanceof Error && "code" in readErr && (readErr as NodeJS.ErrnoException).code !== "ENOENT") {
                  console.error(`✕ cannot read existing ${f.path}: ${readErr instanceof Error ? readErr.message : String(readErr)}`);
                  process.exit(1);
                }
              }
              ...
              console.error(`✕ failed to write ${f.path}: ${err instanceof Error ? err.message : String(err)}`);
              process.exit(1);
            }
          }
      ...
      const genericConfig = JSON.stringify(
        { mcpServers: { nexus: { url: `${origin}/api/mcp`, headers: { Authorization: `Bearer ${apiKey}` } } } },
        null, 2
      );
      if (apply) {
