...
export async function handleMcp(req: IncomingMessage, res: ServerResponse): Promise<boolean> {
  ...
  if (
    origin &&
    env.NEXUS_ALLOWED_ORIGINS.split(',')
      .map((s) => s.trim())
  ...
  if (req.method !== 'POST') {
    send(res, 405, {
      jsonrpc: '2.0',
      error: { code: -32601, message: 'Only POST is supported (stateless MCP).' },
      id: null,
    });
    return true;
  }
  ...
  if (!rl.allowed) {
    send(res, 429, {
      ...
      error: {
        code: -32000,
        message: `Rate limit of ${env.NEXUS_RATE_LIMIT_PER_MINUTE}/min exceeded.`,
      },
      id: null,
    });
    return true;
  ...
  if (len && len > env.NEXUS_MAX_BODY_BYTES) {
    send(res, 413, {
      jsonrpc: '2.0',
      error: { code: -32000, message: `Body exceeds ${env.NEXUS_MAX_BODY_BYTES} bytes.` },
      id: null,
    });
    return true;
  }
  ...
  if (!principal) {
    send(res, 401, {
      ok: false,
      error: { code: 'UNAUTHORIZED', message: 'MCP requires a valid API key.' },
    });
    return true;
  }
  ...
  if (body == null) {
    send(res, 400, {
      jsonrpc: '2.0',
      error: { code: -32700, message: 'Parse error: empty or invalid JSON body.' },
      id: null,
    });
    return true;
  }
