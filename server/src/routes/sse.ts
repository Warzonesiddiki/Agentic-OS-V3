...
sse.post('/api/v1/events/token', async (c) => {
  const bearerKey = c.req
    .header('authorization')
    ?.replace(/^Bearer\s+/i, '')
    .trim();
  if (!bearerKey)
    return c.json(
      err('UNAUTHORIZED', 'Authorization header required.', c.get('requestId') ?? ''),
      401
    );
  ...
  if (!principal)
    return c.json(err('UNAUTHORIZED', 'Invalid API key.', c.get('requestId') ?? ''), 401);
...
sse.get('/api/v1/events', async (c) => {
  ...
  if (token) {
    ...
    if (!data || data.expiresAt <= Date.now()) {
      ...
      return c.json(
        err('UNAUTHORIZED', 'SSE token expired or invalid.', c.get('requestId') ?? ''),
        401
      );
    }
  } else {
