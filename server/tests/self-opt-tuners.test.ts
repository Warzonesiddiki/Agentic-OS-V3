...
/**
 * Unit tests for Phase 18 tuners: shared math + representative propose()/evaluate() behavior.
...
function baseSnapshot(over: Partial<TelemetrySnapshot> = {}): TelemetrySnapshot {
  const snap: TelemetrySnapshot = {
    ...
    recall: { ndcg10: 0.9, missRate: 0.05, weights: { rrf: 0.4, importance: 0.3, recency: 0.2, feedback: 0.1 }, rrfK: 60, hitRate: 0.8 },
...
describe("tuners — propose/evaluate", () => {
  ...
  it("SchedulerPidTuner proposes when queue wait elevated", async () => {
    ...
    const snap = baseSnapshot({
      scheduler: { pid: { kp: 1, ki: 0.1, kd: 0.01 }, queueDepth: 10, queueWaitMs: 500, queueRejectRate: 0.05, boostMs: 5000, policy: "mlfq" },
    });
  ...
  it("RLSchedulingPolicy proposes mlfq with reduced wait", async () => {
    ...
    const snap = baseSnapshot({
      scheduler: { pid: { kp: 1, ki: 0.1, kd: 0.01 }, queueDepth: 10, queueWaitMs: 400, queueRejectRate: 0.02, boostMs: 5000, policy: "mlfq" },
    });
    const d = await t.propose(snap);
    expect(d).not.toBeNull();
