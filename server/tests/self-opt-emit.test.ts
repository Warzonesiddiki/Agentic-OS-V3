...
// Mock the DB client + logging so the controller's persistResult path never
// touches Postgres. We assert the emit path via MetricStore, not via DB rows.
...
import { guardrailGuard, setGuardrailBounds, DEFAULT_BOUNDS, getGuardrailBounds } from '../src/services/self-opt/guardrail-guard.js';
import { metaOptimize } from '../src/services/self-opt/gap-items.js';
import { ALL_TUNERS } from '../src/services/self-opt/tuners.js';

// Pull spies out of the mocked modules (imported after vi.mock hoisting).
...
function allSpies(): Array<ReturnType<typeof vi.fn>> {
  return [
    ...
    rankingMod.applyRankingWeights as unknown as ReturnType<typeof vi.fn>,
...
// A snapshot that trips MANY tuners (latency/queue budget breaches everywhere).
function breachSnapshot(): void {
  metricStore.set('scheduler_queue_depth', 80);
  metricStore.set('scheduler_queue_wait_ms', 900);
  metricStore.set('recall_miss_rate', 0.4);
  metricStore.set('recall_hit_rate', 0.5);
  metricStore.set('prompt_judge_score', 0.3);
  metricStore.set('provider_p99_ms', 5000);
  metricStore.set('agent_oom_count', 3);
  metricStore.set('cache_warm_hit_rate', 0.4);
  metricStore.set('guardrail_violation_rate', 0.3);
  metricStore.set('billing_token_cost_usd', 50);
  metricStore.set('audit_trail_count', 5);
}
...
describe('Pulse: self-opt EMIT path', () => {
  it('(a) emits telemetry + audit events when a tuner flag flips (live cycle)', async () => {
    // Live controller: latency/queue breaches -> proposals allowed to apply.
    breachSnapshot();
  ...
  it('(b) triggers the configured remediation adapter (owner live setter invoked)', async () => {
    ...
    const anySpyCalled = allSpies().some((s) => (s as ReturnType<typeof vi.fn>).mock.calls.length > 0);
    expect(anySpyCalled).toBe(true);

    // Spot-check a deterministic mapping: queue depth breach -> Forge's
    // configureWorker(maxConcurrency) via the queue auto-scaler adapter.
    expect((taskWorkerMod.configureWorker as ReturnType<typeof vi.fn>).mock.calls.length).toBeGreaterThan(0);

    // Guardrail auto-tune -> Sentinel's setGuardrailThreshold via seam.
  ...
  it('(c) respects the daily write budget cap across multiple live cycles (no over-apply)', async () => {
    ...
    // Run many cycles; each breaches budget so every proposal wants to apply.
    let totalApplied = 0;
    let totalProposed = 0;
    for (let i = 0; i < 10; i++) {
      breachSnapshot();
    ...
    expect(getGuardrailBounds().L0_BUDGET).toBe(CAP);
  });
