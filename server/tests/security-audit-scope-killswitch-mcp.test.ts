...
describe('SecD: API-key scope enforcement (requireScope)', () => {
  ...
  it('allows a key holding the wildcard "admin" scope for any route scope', async () => {
    const app = appEnforcingScope('memory:write');
    app.use('/x', (c, next) => {
      c.set('auth', principalWithScopes(['admin']));
      return next();
    });
    const res = await app.request('/x');
    expect(res.status).toBe(200);
...
describe('SecD: hasScope wildcard contract', () => {
  ...
  it('admin grants everything', () => {
    expect(hasScope(['admin'], 'memory:write')).toBe(true);
    expect(hasScope(['admin'], 'kernel:write')).toBe(true);
  });

  it('family wildcard grants family members only', () => {
    expect(hasScope(['memory:*'], 'memory:read')).toBe(true);
    expect(hasScope(['memory:*'], 'memory:write')).toBe(true);
    expect(hasScope(['memory:*'], 'agent:read')).toBe(false);
  });

  it('exact match works', () => {
    expect(hasScope(['agent:read'], 'agent:read')).toBe(true);
    expect(hasScope(['agent:read'], 'agent:write')).toBe(false);
  });
});
