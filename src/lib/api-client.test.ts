...
import { request, configureClient, getApiKey, ApiTimeoutError } from './api-client';

function mockFetchOnce(impl: (input: string, init: RequestInit) => Response | Promise<Response>) {
  const fn = vi.fn(async (input: string, init: RequestInit) => impl(input, init));
...
describe('api-client request — happy path + headers', () => {
  ...
  it('sends the bearer token and parses an ok envelope', async () => {
    ...
    expect(sentHeaders.get('authorization')).toBe('Bearer secret-123');
    configureClient('', '');
  });
